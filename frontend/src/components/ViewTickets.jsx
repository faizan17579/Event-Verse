...
import { FaDownload, FaCreditCard, FaTicketAlt, FaDollarSign, FaArrowLeft } from "react-icons/fa";

// Load Stripe
const stripePromise = loadStripe("pk_test_51QS0yZJDUj9ArTtKWG5Espc1cGGzqoVEsEhaktzAvfT5NXAM6q6ZVEZAvA7TMEZsvZeqIz09PHtfpdoWxPmdwiSv006wFVDKpE");
...
const ViewTickets = () => {
  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      const user = JSON.parse(localStorage.getItem("user"));
      if (!user?.email) throw new Error("Please log in to complete ticket booking.");
  
  
      const response = await fetch("http://localhost:5000/api/tickets/paymentsuccess", {
        method: "POST",
        headers: {
